// Fixed tests for Narrative Awareness System and Level 5 Capabilities
...
async function runAllTests() {
  ...
  // Test 1: TNA Service (with proper LibrosaAnalysis)
  ...
  try {
    ...
    const mockLibrosa = {
      ...
      energy: [0.5, 0.6, 0.7],
      sections: [
        { startTime: 0, endTime: 10, type: 'intro' as const, energy: 'low' as const },
        { startTime: 10, endTime: 20, type: 'verse' as const, energy: 'medium' as const },
        { startTime: 20, endTime: 30, type: 'chorus' as const, energy: 'high' as const },
      ],
      beats: [0, 0.5, 1, 1.5, 2],
      key: 'C major',
    };
    const result = await narrativeTnaService.breakdownToTNAs(testLyrics, mockLibrosa);
    console.log(`   ✓ TNA: Generated ${result.length} atoms`);
    passed++;
  } catch (e: any) {
  ...
  try {
    ...
    const violations = narrativeDtgService.checkConsistency(graph);
    console.log(`   ✓ DTG: Built graph, ${violations.length} violations found`);
    passed++;
  } catch (e: any) {
  ...
  try {
    ...
    const hasCombined = typeof narrativeMetricsService.calculateCombinedScore === 'function';
    console.log(`   ✓ Metrics: NC=${hasNCScore}, SF=${hasSFScore}, Combined=${hasCombined}`);
    passed++;
  } catch (e: any) {
  ...
  // Test 7: Context Contracts (with proper async)
  console.log('\n7. Context Contracts Service...');
  try {
    const contractId = await contextContractsService.createContract('test-pkg-3', 'test-job-3', 0);
    ...
    if (contract) {
      console.log(`   ✓ Context Contracts: ${contractId.substring(0, 30)}..., decisions=${contract.decisions.length}`);
      passed++;
    } else {
